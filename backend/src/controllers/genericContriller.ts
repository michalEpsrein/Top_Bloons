...
async function isStorageLimitExceededFor(maxStorageGB: number) {
  try {
    ...
    console.log(`Cloudinary Storage: ${usedStorageGB.toFixed(2)} GB / ${maxStorageGB} GB`);
...
const extractPublicId = (req: Request | any): string | undefined => {
  return (
    req.params?.publicId || req.params?.id || req.query?.publicId || req.query?.id || req.body?.publicId || undefined
  );
};
...
export function upload(opts: CrudOptions) {
  ...
  return async (req: Request | any, res: Response) => {
    // try {
    //   console.log("--- generic.create called ---");
    //   console.log("content-type:", req.headers?.["content-type"]);
    //   console.log("req.files keys:", req.files ? Object.keys(req.files) : req.files);
    //   console.log("req.body keys:", req.body ? Object.keys(req.body) : req.body);
    // } catch (e) {
    //   console.log("Error logging request debug info", e);
    // }
    ...
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024)
      return res.status(400).json({ message: `File exceeds size limit (${MAX_FILE_SIZE_MB}MB).` });

    const limitExceeded = await isStorageLimitExceededFor(MAX_STORAGE_LIMIT_GB);
    if (limitExceeded) return res.status(403).json({ message: "Storage limit exceeded." });

    try {
      const fileToUpload = file.tempFilePath || file.data;
      const tags = [...(opts.defaultTags || []), ...(body.category ? [body.category] : [])];
      const context = { ...(body || {}) };

      const rawFolderSource = (body.folder || body.owner || body.uploader || body.userId) as string | undefined;
      const senderPart = sanitizeFolderName(rawFolderSource);
      const targetFolder = senderPart ? `${opts.folder}/${senderPart}` : opts.folder;
      ...
      return res.status(201).json({ message: "Uploaded.", data: { publicId, imageUrl, context, folder: targetFolder } });
    } catch (error) {
...
export function getAll(opts: CrudOptions) {
  return async (req: Request, res: Response) => {
    try {
      const searchResult = await cloudinary.search.expression(`folder:${opts.folder}`).with_field("context").with_field("tags").max_results(500).execute();
      const items = (searchResult.resources || []).map((r: any) => ({ id: r.public_id, url: r.secure_url, tags: r.tags, context: r.context }));
      return res.status(200).json({ message: "OK", count: items.length, items });
    } catch (error) {
...
export function get(opts: CrudOptions) {
  return async (req: Request | any, res: Response) => {
    ...
    try {
      ...
      if (error.http_code === 404) return res.status(404).json({ message: "Not found" });
...
export function remove(opts: CrudOptions) {
  return async (req: Request | any, res: Response) => {
    ...
    try {
      ...
      if (result.result === "ok") return res.status(200).json({ message: "Deleted" });
      if (result.result === "not found") return res.status(404).json({ message: "Not found" });
      return res.status(500).json({ message: `Delete failed: ${result.result}` });
    } catch (error) {
